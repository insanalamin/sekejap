...
//!
//! ```rust,no_run
//! use hsdl_sekejap::{SekejapDB, EdgeType};
//! use std::path::Path;
//!
...
//! let results = db.traverse("jakarta-crime-2024", 3, 0.5)?;
//! # Ok(())
//! # }
//! ```
...
pub use types::{
    // Core types
    NodeHeader, NodePayload, Tombstone,
    node::{NodeId, SlugHash, SpatialHash, Epoch, Coordinates, SpatialResult},
    edge::{WeightedEdge, EdgeType, EdgePayload},
    blob::{BlobPtr, BlobStore},
    payload::{Payload, SerializablePayload},
    WriteOptions, ReadOptions, DeleteOptions,
    Point, Polygon, Polyline, Geometry,
    point_in_polygon, polyline_intersects_polygon, distance,
    // DATA_FORMAT.md types
    CollectionId, EntityId, EdgeRef, Collection, parse_entity_id,
    VectorChannel, VectorStore,
    GeoGeometry, GeoFeature, GeoStore,
    Props, TemporalDecay,
    CollectionSchema, HotFields, VectorSchema, SpatialSchema, GeoType, CollectionRegistry,
};

pub use storage::{IngestionBuffer, ServingLayer, SingleStorage, PersistentStorage, PromoteWorker, PromotionMetrics, WorkerCommand, GarbageCollector, GcConfig, GcMetrics, BatchUpsert};
pub use index::{SlugIndex};

#[cfg(feature = "vector")]
pub use vectors::{
    VectorSearchResult,
    quantization::{QuantizedVector, QuantizationType, quantize, dequantize, quantization_error},
    ops::{brute_force_search, vector_to_bytes, bytes_to_vector},
    index::{VectorIndex, IndexBuildPolicy},
};
...
pub use index::{FulltextIndex, FulltextConfig, FulltextResult, FulltextStats};
...
/// ```
    ...
    pub fn write_with_options(&mut self, slug: &str, data: &str, opts: WriteOptions) -> Result<NodeId, Box<dyn std::error::Error>> {
        // Parse JSON data
    ...
    /// # let mut db = SekejapDB::new(Path::new("./data")).unwrap();
    /// db.write_many(vec![
    ///     ("crime-001", r#"{"title": "Theft 1", "coordinates": {"lat": -6.2, "lon": 106.8}}"#),
    ///     ("crime-002", r#"{"title": "Theft 2", "coordinates": {"lat": -6.3, "lon": 106.9}}"#),
    ///     ("crime-003", r#"{"title": "Theft 3", "coordinates": {"lat": -6.4, "lon": 107.0}}"#),
    /// ])?;
    /// # Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
    pub fn write_many(&mut self, items: Vec<(String, String)>) -> Result<Vec<NodeId>, Box<dyn std::error::Error>> {
    ...
    pub fn write_batch(&mut self, items: Vec<(String, String)>, publish_now: bool) -> Result<Vec<NodeId>, Box<dyn std::error::Error>> {
        ...
        for (slug, data) in &items {
            ...
            let mut payload = crate::types::NodePayload::new(title);
            payload.slug = Some(slug.clone());
            payload.content = Some(data.clone());
            payload.metadata = Some(value);
            payload.coordinates = Some(crate::types::Coordinates { latitude: lat, longitude: lon });
            let payload_json = serde_json::to_string(&payload)?;
            let payload_ptr = self.blob_store.write(payload_json.as_bytes())?;
            
            // Create entity_id
            let entity_id = Some(crate::types::EntityId::new("nodes".to_string(), slug.clone()));
    ...
    fn write_node_json(&mut self, value: &serde_json::Value) -> Result<NodeId, Box<dyn std::error::Error>> {
        ...
        let entity_id = value.get("_id").and_then(|v| v.as_str())
            .map(EntityId::parse)
            .transpose()?
            .or_else(|| {
        ...
        let slug_for_hash = entity_id.as_ref()
            .map(|id| id.key())
            .or(value.get("slug").and_then(|v| v.as_str()))
            .unwrap_or(title);
        
        let slug_hash = hash_slug(slug_for_hash);
        
        // Extract coordinates for spatial hash (legacy support)
        let lat = value.get("coordinates").and_then(|c| c.get("lat").and_then(|v| v.as_f64()))
            .or(value.get("latitude").and_then(|v| v.as_f64()))
            .unwrap_or(0.0);
        let lon = value.get("coordinates").and_then(|c| c.get("lon").and_then(|v| v.as_f64()))
    ...
    fn write_edge_json(&mut self, value: &serde_json::Value) -> Result<NodeId, Box<dyn std::error::Error>> {
        ...
        let from_str = value.get("_from").and_then(|v| v.as_str())
            .ok_or("Missing _from in edge")?;
        let to_str = value.get("_to").and_then(|v| v.as_str())
            .ok_or("Missing _to in edge")?;
        
        let _from = EntityId::parse(from_str)
            .map_err(|_| format!("Invalid _from format: {}", from_str))?;
        let _to = EntityId::parse(to_str)
            .map_err(|_| format!("Invalid _to format: {}", to_str))?;
        
        // Extract _type
        let _type = value.get("_type").and_then(|v| v.as_str())
            .or(value.get("type").and_then(|v| v.as_str()))
            .unwrap_or("related")
            .to_string();
        
        // Extract weight from props
        let weight = value.get("props").and_then(|p| p.get("weight"))
            .or(value.get("weight"))
            .and_then(|v| v.as_f64())
            .unwrap_or(1.0) as f32;
        
        // Extract timestamp
        let timestamp = value.get("props").and_then(|p| p.get("timestamp").or(p.get("timestamp")))
            .and_then(|v| v.as_u64())
            .or_else(|| {
        ...
        if let Some(decay_config) = value.get("props").and_then(|p| p.get("decay"))
            .or(value.get("decay"))
            && let serde_json::Value::Object(map) = decay_config {
                let enabled = map.get("enabled").and_then(|v| v.as_bool()).unwrap_or(true);
                let half_life = map.get("half_life_days").and_then(|v| v.as_u64()).unwrap_or(30) as u32;
                let min_weight = map.get("min_weight").and_then(|v| v.as_f64()).unwrap_or(0.1) as f32;
                ...
                let decay = TemporalDecay { enabled, half_life_days: half_life, min_weight, function };
        ...
        let from_id = self.storage.get_by_slug(from_hash)
            .ok_or(format!("Source node not found: {}", _from))?
            .node_id;
        let _to_id = self.storage.get_by_slug(to_hash)
...
/// ```
    ...
    /// # Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
    pub fn traverse(&self, slug: &str, max_hops: usize, weight_threshold: f32) -> Result<TraversalResult, Box<dyn std::error::Error>> {
    ...
    /// # Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
    pub fn add_edge(&mut self, source_slug: &str, target_slug: &str, weight: f32, edge_type: EdgeType) -> Result<(), Box<dyn std::error::Error>> {
        ...
        } else if self.storage.get_by_slug(source_hash).is_some() {
            EntityId::new("nodes".to_string(), source_slug.to_string())
        } else {
        ...
        } else if self.storage.get_by_slug(target_hash).is_some() {
            EntityId::new("nodes".to_string(), target_slug.to_string())
        } else {
    ...
    pub fn restore(&mut self, path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(nodes) = backup_data["nodes"].as_array() {
            for node_data in nodes {
                ...
                let node = NodeHeader::new(
                    ...
                    node_data["spatial_hash"].as_u64().ok_or("Missing spatial_hash")?,
                    payload_ptr,
        ...
        if let Some(edges) = backup_data["edges"].as_array() {
            for edge_data in edges {
                let from_str = edge_data["from"].as_str()
                    .ok_or("Missing 'from' in edge")?;
                let to_str = edge_data["to"].as_str()
                    .ok_or("Missing 'to' in edge")?;
                ...
                let edge_type = edge_data["edge_type"].as_str()
                    .unwrap_or("related")
                    .to_string();
                let evidence_ptr = edge_data["evidence_ptr"].as_u64().unwrap_or(0);
                let valid_start = edge_data["valid_start"].as_u64()
                    .unwrap_or_else(|| {
                        std::time::SystemTime::now()
                            .duration_since(std::time::UNIX_EPOCH)
                            .unwrap()
...
/// ```
    ...
    pub fn delete_with_options(&mut self, slug: &str, opts: DeleteOptions) -> Result<(), Box<dyn std::error::Error>> {
        let slug_hash = hash_slug(slug);
        ...
        if let Some(node) = self.storage.get_by_slug(slug_hash) {
            node_id = Some(node.node_id);
            self.storage.delete_by_slug(slug_hash, Some("user_delete".to_string()));
        }
    ...
    /// # Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
    pub fn delete_edge(&self, source_slug: &str, target_slug: &str, edge_type: Option<EdgeType>) -> Result<bool, Box<dyn std::error::Error>> {
        let source_hash = hash_slug(source_slug);
        let target_hash = hash_slug(target_slug);
        
        // Check if nodes exist (Tier 1 or Tier 2)
        let _from = if self.ingestion.get_by_slug(source_hash).is_some() || self.storage.get_by_slug(source_hash).is_some() {
            EntityId::new("nodes".to_string(), source_slug.to_string())
        } else {
            return Err("Source node not found".into());
        };
        
        let _to = if self.ingestion.get_by_slug(target_hash).is_some() || self.storage.get_by_slug(target_hash).is_some() {
            EntityId::new("nodes".to_string(), target_slug.to_string())
        } else {
            return Err("Target node not found".into());
        };
    ...
    pub fn update(&mut self, slug: &str, data: &str) -> Result<NodeId, Box<dyn std::error::Error>> {
        ...
        let existing_node = self.storage
    ...
    /// # Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
    pub fn update_edge(&mut self, source_slug: &str, target_slug: &str, new_weight: f32, edge_type: Option<EdgeType>) -> Result<bool, Box<dyn std::error::Error>> {
        // Remove old edge
        ...
        if let Some(collections) = value.get("collections").or(Some(&value)) {
            if let serde_json::Value::Object(map) = collections {
                for (collection_id, schema_value) in map {
                    ...
                    self.collection_registry.register(collection_id.clone(), schema);
                }
            }
        }
