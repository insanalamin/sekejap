...
//!
//! ```rust
//! use hsdl_sekejap::types::{VectorChannel, VectorStore};
//!
//! // Dense vector
...
///
/// ```rust
/// use hsdl_sekejap::types::VectorChannel;
///
...
///
/// ```rust
/// use hsdl_sekejap::types::{VectorStore, VectorChannel};
///
/// let mut store = VectorStore::new();
///
/// // Add dense vector
...
impl VectorStore {
    ...
    pub fn estimated_bytes(&self) -> usize {
        self.0.values().fold(0, |acc, v| {
            ...
            let token_bytes = v.tokens.as_ref().map_or(0, |t| t.iter().map(|v| v.len() * 4).sum());
...
impl fmt::Display for VectorChannel {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.is_dense() {
            ...
            write!(f, "VectorChannel({} tokens, {} dims)", self.model, token_count)
        }
    }
}
