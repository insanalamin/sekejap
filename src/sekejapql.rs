...
use crate::{SekejapDB, NodeId, EntityId, atoms::*};
...
/// # use hsdl_sekejap::sekejapql::SekejapQL;
/// # use std::path::Path;
...
/// # Example
/// ```rust,no_run
/// # use hsdl_sekejap::sekejapql::SecurityLimits;
...
impl<'db> SekejapQL<'db> {
    ...
    fn apply_filters(&self, nodes: Vec<NodeResult>, filters: &[FilterSpec]) -> Result<Vec<NodeResult>, QueryError> {
        ...
        match filter {
            ...
            FilterSpec::Eq { field, value } => {
                ...
                if let Some(field_value) = self.get_field_value(node, field) {
                    ...
                    if let (Some(field_str), Some(value_str)) = (field_value.as_str(), value.as_str()) {
                        return field_str == value_str;
                    }
                    // Numeric comparison
                    if let (Some(field_num), Some(value_num)) = (field_value.as_f64(), value.as_f64()) {
            ...
            FilterSpec::Gt { field, value } => {
                self.compare_numeric(node, field, *value, "gt")
            },
            
            FilterSpec::Lt { field, value } => {
                self.compare_numeric(node, field, *value, "lt")
            },
            
            FilterSpec::Gte { field, value } => {
                self.compare_numeric(node, field, *value, "gte")
            },
            
            FilterSpec::Lte { field, value } => {
                self.compare_numeric(node, field, *value, "lte")
            },
            
            FilterSpec::In { field, values } => {
                self.is_in_values(node, field, values)
            },
            
            FilterSpec::Not { filter } => {
                !self.matches_filter(node, filter)
            },
            
            FilterSpec::And { filters } => {
                filters.iter().all(|f| self.matches_filter(node, f))
            },
            
            FilterSpec::Or { filters } => {
                filters.iter().any(|f| self.matches_filter(node, f))
            },
    ...
    fn apply_spatial_filter(&self, nodes: Vec<NodeResult>, _spatial: &SpatialSpec) -> Result<Vec<NodeResult>, QueryError> {
        // TODO: Implement actual spatial filtering using find_within_radius atom
        Ok(nodes)
    }
    
    /// Apply vector filter
    fn apply_vector_filter(&self, nodes: Vec<NodeResult>, _vector: &VectorSpec) -> Result<Vec<NodeResult>, QueryError> {
        // TODO: Implement actual vector filtering using find_similar_vectors atom
        Ok(nodes)
    }
            ...
            && let Ok(payload_bytes) = self.db.blob_store().read(header.payload_ptr)
                && let Ok(payload) = serde_json::from_slice::<crate::types::NodePayload>(&payload_bytes) {
                    ...
                    match field {
                        ...
                        "_timestamp" => return Some(serde_json::Value::Number(serde_json::Number::from(payload._timestamp))),
                        _ => {
                            // Check props
        ...
        if let Some(field_value) = self.get_field_value(node, field) {
            ...
            if let Some(field_str) = field_value.as_str() {
                return values.iter().any(|v| {
                    v.as_str() == Some(field_str)
                });
            }
            // For numeric comparison if value is a number
            if let Some(field_f64) = field_value.as_f64() {
                return values.iter().any(|v| {
                    v.as_f64().is_some_and(|n| (n - field_f64).abs() < 1e-9)
                });
            }
        }
        false
...
mod tests {
    ...
    fn test_simple_filter_query() {
        ...
        assert_eq!(result.nodes.len(), 1, "Expected 1 node, got {}", result.nodes.len());
    ...
    fn test_traversal_query() {
        ...
        assert_eq!(result.nodes.len(), 3, "Expected 3 nodes (start + bandung + jakarta), got {}", result.nodes.len());
    }
    ...
    fn test_eq_filter() {
        ...
        db.write_with_options("item-1", r#"{"title": "Item 100"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-2", r#"{"title": "Item 200"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-3", r#"{"title": "Item 300"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
    ...
    fn test_gt_filter() {
        ...
        db.write_with_options("item-1", r#"{"title": "Count 10"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-2", r#"{"title": "Count 50"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-3", r#"{"title": "Count 100"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
    ...
    fn test_lt_filter() {
        ...
        db.write_with_options("item-1", r#"{"title": "Score 10"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-2", r#"{"title": "Score 50"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-3", r#"{"title": "Score 100"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
    ...
    fn test_gte_lte_filter() {
        ...
        db.write_with_options("item-1", r#"{"title": "Value 100"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-2", r#"{"title": "Value 150"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-3", r#"{"title": "Value 200"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
    ...
    fn test_in_filter() {
        ...
        db.write_with_options("item-1", r#"{"title": "Type Alpha"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-2", r#"{"title": "Type Beta"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-3", r#"{"title": "Type Charlie"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-4", r#"{"title": "Type Delta"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        ...
        let result = engine.execute(query).unwrap();
        assert_eq!(result.nodes.len(), 3, "Expected 3 nodes with Alpha, Charlie, or Delta in title");
    }
    ...
    fn test_and_filter() {
        ...
        db.write_with_options("item-1", r#"{"title": "X10 Y20"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-2", r#"{"title": "X10 Y50"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-3", r#"{"title": "X30 Y20"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        ...
        let result = engine.execute(query).unwrap();
        assert_eq!(result.nodes.len(), 2, "Expected 2 nodes with X10 AND Y in title");
    }
    ...
    fn test_or_filter() {
        ...
        db.write_with_options("item-1", r#"{"title": "Status active"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-2", r#"{"title": "Status pending"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-3", r#"{"title": "Status rejected"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        ...
        let result = engine.execute(query).unwrap();
        assert_eq!(result.nodes.len(), 2, "Expected 2 nodes with active OR pending in title");
    }
    ...
    fn test_not_filter() {
        ...
        db.write_with_options("item-1", r#"{"title": "Visible true"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("item-2", r#"{"title": "Hidden false"}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        ...
        let result = engine.execute(query).unwrap();
        assert_eq!(result.nodes.len(), 1, "Expected 1 node that does NOT contain 'Hidden' in title");
    }
    ...
    fn test_content_contains_filter() {
        ...
        db.write_with_options("news-1", r#"{"title": "Breaking News: Earthquake", "content": "A major earthquake struck today."}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("news-2", r#"{"title": "Weather Update", "content": "Sunny weather expected tomorrow."}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        db.write_with_options("news-3", r#"{"title": "Finance Report", "content": "Stock market earthquake today."}"#,
            WriteOptions { publish_now: true, ..Default::default() }).unwrap();
        ...
        let result = engine.execute(query).unwrap();
        assert_eq!(result.nodes.len(), 2, "Expected 2 nodes with 'earthquake' in content");
    }
}
