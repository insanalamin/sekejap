...
impl FulltextIndex {
    ...
    pub fn search(&self, query: &str, limit: usize) -> Result<Vec<FulltextResult>, Box<dyn std::error::Error>> {
        ...
        for (score, address) in top_docs {
            ...
            if let Some(title) = doc_json.get("title").and_then(|v: &serde_json::Value| v.as_str()) {
                stored_fields.insert("title".to_string(), title.to_string());
            }
            if let Some(content) = doc_json.get("content").and_then(|v: &serde_json::Value| v.as_str()) {
...
impl FulltextIndex {
    ...
    pub fn add_document(&mut self, _title: &str, _content: &str, _slug: &str) -> Result<(), Box<dyn std::error::Error>> {
        Err("Fulltext feature not enabled. Recompile with --features fulltext".into())
    }
    ...
    pub fn search(&self, _query: &str, _limit: usize) -> Result<Vec<FulltextResult>, Box<dyn std::error::Error>> {
        Err("Fulltext feature not enabled. Recompile with --features fulltext".into())
    }
    ...
    pub fn stats(&self) -> FulltextStats {
        FulltextStats { num_docs: 0, segment_count: 0 }
    }
}
...
mod tests {
    ...
    fn test_single_document() {
        ...
        index.add_document("Test Title", "Test content here", "test-1").unwrap();
        index.commit().unwrap();
    ...
    fn test_basic_search() {
        ...
        index.add_document("Jakarta Crime", "Crime rate increased in Jakarta", "news-1").unwrap();
        index.add_document("Bandung Weather", "Weather is nice in Bandung", "news-2").unwrap();
        index.add_document("Surabaya Traffic", "Traffic jam in Surabaya", "news-3").unwrap();
        index.commit().unwrap();
    ...
    fn test_phrase_search() {
        ...
        index.add_document("Article", "The quick brown fox jumps", "doc-1").unwrap();
        index.add_document("Article", "The lazy dog sleeps", "doc-2").unwrap();
        index.commit().unwrap();
    ...
    fn test_boolean_search() {
        ...
        index.add_document("Doc 1", "apple orange banana", "doc-1").unwrap();
        index.add_document("Doc 2", "apple pear", "doc-2").unwrap();
        index.add_document("Doc 3", "banana grape", "doc-3").unwrap();
        index.commit().unwrap();
    ...
    fn test_field_boost() {
        ...
        index.add_document("Apple Product", "This is about apple the fruit", "doc-1").unwrap();
        index.add_document("Product Review", "Apple makes great products", "doc-2").unwrap();
        index.commit().unwrap();
    ...
    fn test_delete_document() {
        ...
        index.add_document("Test", "Content", "test-1").unwrap();
        index.commit().unwrap();
        assert_eq!(index.num_docs(), 1);
    ...
    fn test_clear_index() {
        ...
        for i in 0..10 {
            index.add_document(&format!("Title {}", i), &format!("Content {}", i), &format!("doc-{}", i)).unwrap();
        }
        index.commit().unwrap();
    ...
    fn test_stemming() {
        ...
        index.add_document("Running", "The program is running smoothly", "doc-1").unwrap();
        index.add_document("Run", "I will run tomorrow", "doc-2").unwrap();
        index.commit().unwrap();
        
        // Both should match "running" due to stemming
        let results = index.search("running", 10).unwrap();
        assert_eq!(results.len(), 2);
    }
}
