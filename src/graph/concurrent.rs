...
use crate::{WeightedEdge, EdgeType, EntityId};
use dashmap::DashMap;
...
impl ConcurrentGraph {
    ...
    pub fn backward_bfs(
        ...
        visited.insert(start_entity_id.clone(), true);
            ...
            if let Some(rev_edges) = self.reverse_edges.get(&entity_id) {
                for edge in rev_edges.value().iter() {
                    ...
                    if let Some((window_start, window_end)) = time_window {
                        ...
                        let edge_valid_in_window = 
                            edge.valid_start < window_end && 
                    ...
                    if !visited.contains_key(&edge._from) {
                        visited.insert(edge._from.clone(), true);
                        queue.push_back((edge._from.clone(), depth + 1, cumulative_weight * edge.weight));
