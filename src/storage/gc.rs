...
impl GarbageCollector {
    ...
    pub fn stop(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(handle) = self.thread_handle.take() {
            handle.join().map_err(|e| format!("GC thread panicked: {:?}", e))?;
        }
    ...
    fn run_gc(
        ...
        loop {
            ...
            let elapsed = last_run.elapsed().unwrap_or_else(|_| Duration::from_secs(0));
    ...
    fn do_gc(
        ...
        let mut versions: std::collections::HashMap<u64, Vec<NodeHeader>> = std::collections::HashMap::new();
        for node in &all_nodes {
            versions.entry(node.slug_hash).or_default().push(node.clone());
        }
                            ...
                            if !config.always_keep_head || node_versions.len() > 1 {
                                for _node in &node_versions {
                                    if deleted_count < config.max_deletes_per_run {
                                        storage.delete_by_slug(slug_hash, Some("gc_delete".to_string()));
                                        deleted_count += 1;
        ...
        // Update metrics
        {
            ...
            m.end_run(start.elapsed().unwrap_or_else(|_| Duration::from_secs(0)).as_millis() as u64);
        }
        ...
        log::info!(
            ...
            compacted_count,
            bytes_reclaimed,
            start.elapsed().unwrap_or_else(|_| Duration::from_secs(0)).as_millis()
        );
    }
}
