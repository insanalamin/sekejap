...
use sekejap::{SekejapDB, GcConfig, WriteOptions};
use std::path::Path;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..10 {
        db.write(
            &format!("event-{}", i),
            &format!(r#"{{"title": "Event {}", "coordinates": {{"lat": -6.2, "lon": 106.8}}}}"#, i)
        )?;
    }
    ...
    for i in 10..20 {
        db.write_with_options(
            &format!("event-{}", i),
            &format!(r#"{{"title": "Event {}", "coordinates": {{"lat": -6.3, "lon": 106.9}}}}"#, i),
    ...
    println!("   - Deletion retention: {} days", gc_config.deletion_retention_sec / 86400);
    println!("   - Max versions: {}", gc_config.max_versions);
    println!("   - GC interval: {} hours", gc_config.gc_interval_sec / 3600);
    println!("   - Max deletes per run: {}", gc_config.max_deletes_per_run);
    ...
    db.update("event-3", r#"{"title": "Event 3 (Updated)", "coordinates": {"lat": -6.2, "lon": 106.8}}"#)?;
    db.update("event-3", r#"{"title": "Event 3 (Updated Again)", "coordinates": {"lat": -6.2, "lon": 106.8}}"#)?;
    db.update("event-3", r#"{"title": "Event 3 (Final)", "coordinates": {"lat": -6.2, "lon": 106.8}}"#)?;
