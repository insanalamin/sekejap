...
use sekejap::{SekejapDB, WriteOptions, quantize, dequantize, QuantizationType};
...
fn test_rag_semantic_search() {
    ...
    let docs = vec![
        ("python-guide", r#"{"title": "Python Programming Guide"}"#,
         vec![0.9, 0.05, 0.05, 0.05, 0.05]),  // Programming
        ("rust-guide", r#"{"title": "Rust Programming Guide"}"#,
         vec![0.85, 0.1, 0.1, 0.05, 0.05]),   // Programming
        ("cooking-recipe", r#"{"title": "Italian Cooking Recipe"}"#,
         vec![0.05, 0.9, 0.05, 0.05, 0.05]),  // Cooking
    ];
...
fn test_rag_with_metadata_filtering() {
    ...
    let docs = vec![
        ("doc-a", r#"{"title": "Doc A", "source": "source1"}"#,
         vec![0.5, 0.5, 0.5, 0.5]),
        ("doc-b", r#"{"title": "Doc B", "source": "source2"}"#,
         vec![0.5, 0.5, 0.5, 0.5]),
    ];
...
fn test_rag_context_retrieval() {
    ...
    let context_chunks = vec![
        ("chunk-1", "The capital of France is Paris.", vec![0.8, 0.2, 0.1, 0.1]),
        ("chunk-2", "Paris is known for the Eiffel Tower.", vec![0.7, 0.3, 0.2, 0.1]),
        ("chunk-3", "The population of Tokyo is about 14 million.", vec![0.1, 0.1, 0.9, 0.1]),
    ];

    for (slug, content, embedding) in &context_chunks {
        let data = format!(r#"{{"title": "{}", "content": "{}", "type": "chunk"}}"#, slug, content);
        db.write_with_options(slug, &data, WriteOptions {
            vector: Some(embedding.clone()),
            publish_now: true,
            ..Default::default()
...
fn test_rag_prompt_construction() {
    ...
    db.write_with_options("context-1", r#"{"title": "Context 1", "content": "Python uses indentation for blocks."}"#,
        WriteOptions {
    ...
    // Retrieve context
    let context_results = db.query()
        .vector_search(vec![0.85, 0.15, 0.1, 0.1], 1)
        .execute()
        .unwrap();

    // Construct RAG prompt
    let system_prompt = "You are a helpful assistant. Use the following context to answer the user's question.";
