...
use sekejap::{SekejapDB, WriteOptions, quantize, dequantize, quantization_error, QuantizationType};
...
fn test_vector_search_basic() {
    ...
    let docs = vec![
        ("doc-1", r#"{"title": "Technology Article"}"#, vec![0.9, 0.1, 0.1, 0.1]),
        ("doc-2", r#"{"title": "Sports Article"}"#, vec![0.1, 0.9, 0.1, 0.1]),
        ("doc-3", r#"{"title": "Politics Article"}"#, vec![0.1, 0.1, 0.9, 0.1]),
        ("doc-4", r#"{"title": "Art Article"}"#, vec![0.1, 0.1, 0.1, 0.9]),
        ("doc-5", r#"{"title": "Tech Sports Mix"}"#, vec![0.5, 0.5, 0.1, 0.1]),
    ];

    for (slug, data, vector) in &docs {
        db.write_with_options(slug, data, WriteOptions {
...
fn test_vector_search_similarity_ordering() {
    ...
    // Exact match
    db.write_with_options("exact", r#"{"title": "Exact Match"}"#, WriteOptions {
    ...
    // Similar
    db.write_with_options("similar", r#"{"title": "Similar"}"#, WriteOptions {
    ...
    // Different
    db.write_with_options("different", r#"{"title": "Different"}"#, WriteOptions {
    ...
    // Search
    let results = db.query()
        .vector_search(base_vector, 3)
        .execute()
        .unwrap();

    assert_eq!(results.len(), 3);
    
    // Verify results contain all 3 documents by checking node_ids
    let exact_node_id = db.storage().get_by_slug(sekejap::hash_slug("exact")).unwrap().node_id;
    let similar_node_id = db.storage().get_by_slug(sekejap::hash_slug("similar")).unwrap().node_id;
    let different_node_id = db.storage().get_by_slug(sekejap::hash_slug("different")).unwrap().node_id;
    
    let result_ids: Vec<_> = results.iter().map(|r| r.node_id).collect();
    assert!(result_ids.contains(&exact_node_id), "Exact should be in results");
    assert!(result_ids.contains(&similar_node_id), "Similar should be in results");
    assert!(result_ids.contains(&different_node_id), "Different should be in results");
}
...
fn test_vector_search_k_limit() {
    ...
    for i in 0..10 {
        db.write_with_options(&format!("doc-{}", i), &format!(r#"{{"title": "Doc {}"}}"#, i), WriteOptions {
...
fn test_multimodal_with_coordinates() {
    ...
    let docs = vec![
        ("resto-1", r#"{"title": "Jakarta Restaurant"}"#, vec![0.9, 0.1], -6.2088, 106.8456),
        ("resto-2", r#"{"title": "Bandung Restaurant"}"#, vec![0.1, 0.9], -6.9175, 107.6191),
    ];
...
fn test_mixed_workload() {
    ...
    let operations = vec![
        ...
        ("node-2", r#"{"title": "With Vector"}"#, Some(vec![0.1, 0.2, 0.3, 0.4])),
        ("node-3", r#"{"title": "Another No Vector"}"#, None),
        ("node-4", r#"{"title": "Another With Vector"}"#, Some(vec![0.9, 0.8, 0.7, 0.6])),
    ];
