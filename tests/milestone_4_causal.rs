...
fn test_backward_bfs_basic() {
    ...
    db.add_edge("rootcause", "intermediate1", 0.8, "causes".to_string()).unwrap();
    db.add_edge("intermediate1", "intermediate2", 0.7, "causes".to_string()).unwrap();
    db.add_edge("intermediate2", "effect", 0.9, "causes".to_string()).unwrap();

    // Traverse backward from effect
    let result = db.traverse("effect", 10, 0.5).unwrap();

    // Should find all nodes in the chain (4 nodes: effect, inter2, inter1, rootcause)
    assert!(result.path.len() >= 4, "Expected at least 4 nodes, got {}", result.path.len());
    assert!(result.edges.len() >= 3, "Expected at least 3 edges, got {}", result.edges.len());
}
...
fn test_weight_threshold_filtering() {
    ...
    db.write_with_options("cause-high", r#"{"title": "High Weight Cause"}"#,
        sekejap::WriteOptions { publish_now: true, ..Default::default() }
    ).unwrap();
    db.write_with_options("cause-low", r#"{"title": "Low Weight Cause"}"#,
        sekejap::WriteOptions { publish_now: true, ..Default::default() }
    ).unwrap();
    ...
    db.add_edge("cause-high", "effect", 0.9, "causes".to_string()).unwrap();
    db.add_edge("cause-low", "effect", 0.3, "causes".to_string()).unwrap();

    // With high threshold, only cause-high should be found
    let result = db.traverse("effect", 10, 0.8).unwrap();
    assert_eq!(result.path.len(), 2); // effect + cause-high

    // With low threshold, both should be found
    let result = db.traverse("effect", 10, 0.2).unwrap();
...
fn test_max_hops_limit() {
    ...
    for (i, slug) in nodes.iter().enumerate() {
        db.write_with_options(slug, &format!(r#"{{"title": "Node {}", "level": {}}}"#, slug, i),
            sekejap::WriteOptions { publish_now: true, ..Default::default() }
        ).unwrap();
    }
    ...
    // Create chain
    for i in 0..4 {
        db.add_edge(&nodes[i], &nodes[i + 1], 0.8, "causes".to_string()).unwrap();
    }

    // Traverse with max_hops=2
    let result = db.traverse("node-4", 2, 0.5).unwrap();
...
fn test_crime_cause_aggregation() {
    ...
    for (slug, cause_slugs) in &crimes {
        db.write_with_options(slug, &format!(r#"{{"title": "Theft {}", "type": "theft"}}"#, slug),
            sekejap::WriteOptions { publish_now: true, ..Default::default() }
        ).unwrap();
    ...
    for (slug, _) in &crimes {
        let result = db.traverse(slug, 10, 0.5).unwrap();
...
fn test_multiple_edge_types() {
    ...
    db.add_edge("source", "target", 0.8, "causes".to_string()).unwrap();
    db.add_edge("source", "target", 0.6, "influences".to_string()).unwrap();

    let result = db.traverse("target", 10, 0.5).unwrap();
...
fn test_empty_graph_traversal() {
    ...
    let result = db.traverse("nonexistent", 10, 0.5).unwrap();

    // Start node is always in path (even if not in graph), but no edges should be found
    assert!(result.edges.is_empty(), "Expected no edges for non-existent node");
}
